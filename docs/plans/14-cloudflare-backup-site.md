...
> 状态：核心架构已上线；本文保存实施范围、阶段与验收历史。当前生产拓扑和长期约束以
> [生产架构](../ARCHITECTURE.md)为唯一事实源。
...
- [x] 本次上线两个站点保持同一主站版本与同一源代码提交；主站 bundle 差异仅来自已登记的
  QQ 安装包部署变量。持续发布仍需逐次核对，Cloudflare deploy manifest 缺口登记在架构文档。
- [ ] QQ 安装包等待项目主确定独立方案；在此之前 Cloudflare 下载入口继续降级为 Toast，
  不启用 R2，也不恢复 Cloudflare 下载开关。
- [x] 建立 [生产架构](../ARCHITECTURE.md)，固化双域名路由、共享/隔离状态、发布顺序、
  故障边界和未来变更检查清单。
- [x] 固化默认双域名同步发布制度：“上线/发布”默认覆盖两个正式域名；只有项目主明确指定
  才允许单域名例外，两边部署和 smoke 均成功后才能宣告完成。
