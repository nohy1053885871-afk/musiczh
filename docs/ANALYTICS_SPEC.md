...
| `transcode_fail` | 主站 - 业务 - 转码失败 | [src/App.tsx](../src/App.tsx) `transcodeFile` catch | `file_name, error_msg, error_stack, transcode_ms, ...` | 同步触发 `trackFailure`。**v0.6.4 起**带 `transcode_ms`（仅诊断，不进性能均值） |
