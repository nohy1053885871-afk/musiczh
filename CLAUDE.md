...
- 当前开发版本：v0.8.3（运营后台 v0.4.16，API v0.4.9）
- 当前生产版本：主站 v0.8.2 · 运营后台 v0.4.15 · API v0.4.9
- 上线状态：生产仍为主站 v0.8.2 · 运营后台 v0.4.15 · API v0.4.9；v0.8.3 / v0.4.16 仅本地实现，尚未发布
...
### v0.8.3 / 运营后台 v0.4.16 · 20260809 待上线
...
- 验证：主站与运营后台构建通过；新增组件和后台映射 lint 通过，App/QQ 组件既有 3 条历史 lint 未扩大；1280×720 与 390×667 浏览器验收覆盖入口与分隔符整组换行、QQ 红色系编号单条幅、帮助弹窗固定 CTA/背景隔离/无横向溢出；按钮、右上角、Esc、遮罩四种关闭 action 全部写入隔离数据库；修复 StrictMode 首次懒加载重复曝光后，每次打开恰好一条 view；后台 v0.4.16 实机显示四个中文事件标签；帮助弹窗独立 chunk gzip 3.02 KiB
