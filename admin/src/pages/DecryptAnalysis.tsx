...
import { Card, Row, Col, Space, Typography, Button, Drawer, Input, Select, Tag, Descriptions, App } from 'antd'
import { ReloadOutlined, CopyOutlined } from '@ant-design/icons'
import type { ColumnsType } from 'antd/es/table'
import {
  api,
  type FailureRow, type FailureDetail,
  type SuccessRow, type SuccessDetail,
  type FormatDistributionResp,
} from '../lib/api'
...
import { formatBytes, formatTime, STAGE_LABEL, extLabel, formatPercent } from '../lib/format'
...
export function DecryptAnalysisPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <FailuresSection rq={rq} reloadKey={reloadKey} />
      <SuccessesSection rq={rq} reloadKey={reloadKey} />
      <FormatDistributionSection rq={rq} reloadKey={reloadKey} />
    </div>
  )
}
...
function FailuresSection({ rq, reloadKey }: { rq: string; reloadKey: number }) {
  ...
  const columns: ColumnsType<FailureRow> = [
    ...
    { title: '阶段', dataIndex: 'stage', key: 'stage', width: 70,
      render: (v: 'decrypt' | 'transcode') =>
        <Tag color={v === 'decrypt' ? 'blue' : 'purple'}>{STAGE_LABEL[v]}</Tag> },
...
function SuccessesSection({ rq, reloadKey }: { rq: string; reloadKey: number }) {
  const [page, setPage] = useState(1)
  const [stage, setStage] = useState('')
  const [ext, setExt] = useState('')
  const [q, setQ] = useState('')
...
function FormatDistributionSection({ rq, reloadKey }: { rq: string; reloadKey: number }) {
